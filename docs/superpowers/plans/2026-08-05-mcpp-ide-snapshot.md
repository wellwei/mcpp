...
| `tests/e2e/185_ide_snapshot.sh` | Fresh-binary CLI contract and workspace/error cases |
| `tests/e2e/186_ide_snapshot_read_only.sh` | Before/after project and isolated-home zero-side-effect proof |
...
- Create: `tests/e2e/185_ide_snapshot.sh`
...
Create `tests/e2e/185_ide_snapshot.sh` with `# requires:` on line 2. It must:
...
MCPP="$FRESH_MCPP" bash tests/e2e/185_ide_snapshot.sh
```
...
```bash
git add src/cli/cmd_ide.cppm src/cli.cppm tests/e2e/01_help_and_version.sh tests/e2e/185_ide_snapshot.sh
...
**Files:**
- Create: `tests/e2e/186_ide_snapshot_read_only.sh`
- Modify: `tests/unit/test_ide_snapshot.cpp`

- [ ] **Step 1: Add the failing side-effect E2E**

Create `tests/e2e/186_ide_snapshot_read_only.sh` with `# requires:` on line 2. The
...
```bash
FRESH_MCPP=$(find "$PWD/target" -type f -path '*/bin/mcpp' -perm -111 | head -1)
MCPP="$FRESH_MCPP" bash tests/e2e/186_ide_snapshot_read_only.sh
```
...
```bash
git add tests/unit/test_ide_snapshot.cpp tests/e2e/186_ide_snapshot_read_only.sh
